...
def local_LM(starting_paths, distances):
    ...
    def calc_total_cost(c1, c2, dist):
        ...
        for path in [c1, c2]:
            for i in range(len(path)):
                cost += dist[path[i]][path[(i + 1) % len(path)]]
        return cost
    ...
    def is_move_applicable(removed_edges, path1, path2):
        current_edges = {(path1[i], path1[(i + 1) % len(path1)]) for i in range(len(path1))}
        current_edges.update((path2[i], path2[(i + 1) % len(path2)]) for i in range(len(path2)))
        all_present = True
        same_direction = True
    ...
    def add_new_moves(LM, path1, path2, distances, recent_move):
        ...
        # Generowanie nowych ruchów na podstawie ostatniego ruchu
            ...
            if recent_move[0] == "swap":
                ...
                indices1 = {i, (i - 1) % len(path1), (i + 1) % len(path1)}
                indices2 = {j, (j - 1) % len(path2), (j + 1) % len(path2)}
                # generujemy możliwe swapy
                for ni in indices1:
                    for nj in range(len(path2)):
                        tmoves.append((ni, nj, "swap"))
                for nj in indices2:
                    for ni in range(len(path1)):
                        tmoves.append((ni, nj, "swap"))
                # generujemy możliwe edge_swapy
                for idx in indices1:
                    for jdx in range(len(path1)):
                        if idx != jdx:
                            tmoves.append((min(idx, jdx), max(idx, jdx), "edge_swap", 1))
                for idx in indices2:
                    for jdx in range(len(path2)):
                        if idx != jdx:
        ...
        for move in tmoves:
            if move[2] == "swap":
                ...
                try:
                    a, b = path1[i], path2[j]
                    prev_i, next_i = path1[(i - 1) % len(path1)], path1[(i + 1) % len(path1)]
                    prev_j, next_j = path2[(j - 1) % len(path2)], path2[(j + 1) % len(path2)]
                except IndexError:
                    continue
    ...
    def update_LM_after_move(LM, last_move, path1, path2, distances):
        ...
        if last_move[0] == "swap":
            ...
            changed_nodes = {
                path1[i], path2[j],
                path1[(i - 1) % len(path1)], path1[(i + 1) % len(path1)],
                path2[(j - 1) % len(path2)], path2[(j + 1) % len(path2)]
            }
            removed_edges = [
                (path1[(i - 1) % len(path1)], path1[i]),
                (path1[i], path1[(i + 1) % len(path1)]),
                (path2[(j - 1) % len(path2)], path2[j]),
                (path2[j], path2[(j + 1) % len(path2)])
            ]
        else:
    ...
    current_cost = calc_total_cost(path1, path2, distances)
    LM = deque()
